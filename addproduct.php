...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($product_name && $category && $sku && $quantity !== false && $price !== false) {
        $conn = new mysqli($servername, $username, $password, $dbname);
        if ($conn->connect_error) {
            die("Connection failed: " . $conn->connect_error);
        }
        $stmt = $conn->prepare("INSERT INTO products (product_name, category, sku, quantity, description, price) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("sssids", $product_name, $category, $sku, $quantity, $description, $price);
        if ($stmt->execute()) {
